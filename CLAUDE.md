...
- **Homepage crawlable for OAuth reviewers (`e84ea9d`)** — `AuthGate` no longer shows the auth spinner on `/` while `getSession()` runs; static export and bots get full `LandingPage` HTML in the body (was spinner-only, blocking Google OAuth homepage verification). Landing header adds Privacy link + hero copy on Google sign-in data use. **Invariant:** `/` body must be marketing HTML at build/SSR time — never gate the homepage behind an auth-loading shell.

- **Structural Flags panel (`5bfa1fc`, `6eef471`)** — New deterministic, document-level ATS / formatting checks shown as a dedicated **"Structural Flags"** panel in the Analyze overview (Issue + Risk cards with a severity dot), distinct from the LLM's per-bullet categories *and* from `topIssues`. `resume_gui/analysis/structural_flags.py` → `compute_structural_flags(raw_text, structured)` flags: missing LinkedIn; missing **or state-only** location on the current role (e.g. "New Jersey, NJ" has no city — ambiguous "New York"/"Washington" excluded); open/single-date roles; community/club entries misclassified under Experience; `·` **and em-dash** field separators (bullet prose excluded by length/glyph guards); and a **broken text layer** (`6eef471`) — 3+ `(cid:NN)` glyph codes in the extract means a text-based ATS gets scrambled content (HIGH severity; this is also what hid Ria's real separators behind glyph codes). It reads the **original** extracted text (so formatting isn't normalized away by synthesis) plus the structured doc. Wired in `routes/analyze.py` → `result["structuralFlags"]`; rendered in `AnalyzeResume.tsx`. Closes the gap where these were computed by `_recruiter_checks` but only passed to the prompt as hints and never surfaced. Verified live (Ria → 6 flags matching a hand-audit). +19 tests (`test_structural_flags.py`, suite 131). **Invariant:** structural flags are deterministic and live in their own panel — never merged into bullet `topIssues` (keeps the deterministic-insight gating rationale satisfied), and their copy avoids em-dashes (inv. #16).
...
- **Token usage admin dashboard (`uncommitted`)** — Three-layer feature: (1) **`resume_gui/services/usage.py`** adds `log_usage_event()` — a fire-and-forget helper that writes to the `usage_events` Supabase table (already in schema, was never written to). (2) **`resume_gui/llm/client.py`** updated: `_llm_json_call` captures `r.usage` / `r.usage_metadata` from Grok (OpenAI SDK) and Gemini responses into a `_usage` closure dict, accepts optional `_log_user_id/_log_email/_log_tool` kwargs, and calls `log_usage_event` via a `_maybe_log` helper before returning. `_analyze_resume_comprehensive` passes these through; both analyze routes (`api_analyze_upload` and `api_analyze`) pass `auth_user_id/auth_user_email` into the analysis call via `partial(...)`. (3) **`resume_gui/routes/admin.py`** — new `GET /api/admin/analytics` route (global_admin only via `_advisor_scope_for_request`) that queries `usage_events` for token-level aggregations (per-user, per-tool, per-model, daily) AND `resume_analyses` as a supplemental activity source. (4) Frontend **`web/app/admin/analytics/page.tsx`** completely rewritten: KPI strip (LLM runs, tokens with in/out split, analyses, active users, failure rate, models), a `computeInsights()` function that generates analyst commentary cards (token concentration Pareto signal, failure rate assessment, most-expensive-tool-per-run ratio, completion bloat signal, growth trend from daily series, token attribution gap), daily sparkline, endpoint cost breakdown table with `tokens_per_run` colored red/amber/normal, per-user token table with sort + mini bars + % share, and an activity-by-user table sourced from `resume_analyses`. **Auth sentinel fix**: `bearerToken` uses `undefined` (not yet resolved) vs `null` (no session) so the fetch always fires. **Invariant:** `usage_events` is append-only by the backend; frontend never writes to it. The activity table from `resume_analyses` is always populated even before token logging reaches production.

---

*Last updated: 2026-06-05 — token usage admin dashboard. Next session: read this file first, update it last.*
