...
- **Structural Flags panel (`uncommitted`)** — New deterministic, document-level ATS / formatting checks shown as a dedicated **"Structural Flags"** panel in the Analyze overview (Issue + Risk cards with a severity dot), distinct from the LLM's per-bullet categories *and* from `topIssues`. `resume_gui/analysis/structural_flags.py` → `compute_structural_flags(raw_text, structured)` flags: missing LinkedIn, missing location on the current role, open/single-date roles, community/club entries misclassified under Experience, and `·` separators. It reads the **original** extracted text (so separators aren't normalized away by synthesis) plus the structured doc. Wired in `routes/analyze.py` → `result["structuralFlags"]`; rendered in `AnalyzeResume.tsx`. Closes the gap where these (e.g. "No LinkedIn", "Carrom Club under Experience") were computed by `_recruiter_checks` but only passed to the prompt as hints and never surfaced. Verified live on a real résumé (4 flags matching a hand-audit). +11 tests (`test_structural_flags.py`, suite 123). **Invariant:** structural flags are deterministic and live in their own panel — never merged into bullet `topIssues` (keeps the deterministic-insight gating rationale satisfied), and their copy avoids em-dashes (inv. #16).
