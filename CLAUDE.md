...
- **`4cf882a`** — **Section box-wise editing feature enabled.** The feature was implemented in previous commits (`ee527c2` and `ddc3408`) but blocked by `presentationOnly={true}` on `AnalyzePreviewPane`. Fixed by setting `presentationOnly={false}` in `AnalyzeResume.tsx` line 2403. This enables users to click on section headings (EXPERIENCE, EDUCATION, SKILLS, etc.) to select and edit them inline with a textarea, with visual feedback (accent border + "[Click to edit]" hint). Non-destructive: press Escape to cancel, click away to save. Feature is frontend-only; no database persistence in current version.

---

*Last updated: 2026-06-13 — section-editing feature enabled (`4cf882a`). Next session: read this file first, update it last.*
