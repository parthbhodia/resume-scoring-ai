...
- **Email marketing: Brevo + Resend integration (`fb53ebc` + `baf9e72` on main)** — Two-service email stack wired to the Settings tab "Email Preferences" checkboxes. **Brevo** (marketing) — `resume_gui/services/brevo.py` `subscribe()`/`unsubscribe()`/`sync()` against the contacts API (env: `BREVO_API_KEY`, `BREVO_MARKETING_LIST_ID`; live list ID = **3**, "Resunova Users"). **Resend** (transactional) — `resume_gui/services/resend_email.py` `send()` + pre-built `send_scan_limit_warning` / `send_welcome` templates (env: `RESEND_API_KEY`, `RESEND_FROM_EMAIL`; the prod key is send-scoped). **Route** — `resume_gui/routes/notify_prefs.py` `GET/POST /api/profile/notify-prefs`: persists `{accountChanges, scanLimit, features}` to `user_profiles.notify_prefs` (JSONB), then syncs the `features` flag to Brevo (subscribe when on, unsubscribe when off). **DB** — migration `web/db/migrations/012_notify_prefs.sql` adds the `notify_prefs` JSONB column; **applied + verified on the live Supabase project `uvnncrtulyezsylcxnhw`** (the one `.env.dev` points to). **Frontend** — `web/components/profile/SettingsTab.tsx` loads prefs from the backend on mount (DB overrides localStorage), debounced 600ms POST on change, "Saving… / ✓ Saved" indicator in the card header. Also fixed a layout bug in the same file's checkbox row: a global `input { width: 100% }` rule in `globals.css` was swamping Tailwind `w-4` — excluded `input[type=checkbox/radio]` from the form-element rules (commit `c33e480`). **Invariants:** (1) all Brevo/Resend calls are fire-and-forget — failures are logged, never raised, so a down email provider never breaks pref-saving; (2) Brevo requires "Authorised IPs" **disabled** in account security (Railway egress IPs are dynamic — verified end-to-end: subscribe → 201, list → 1, unsubscribe → 204, delete → 0). **Not yet deployed:** Railway needs the 4 env vars; the `web/` SettingsTab change must deploy to staging (the local preview runs the older `resunova-web` symlink, so this UI isn't testable in the current preview).
...
*Last updated: 2026-06-13 — Email marketing (Brevo + Resend) wired to Settings email prefs; `notify_prefs` column applied to live Supabase (`fb53ebc` + `baf9e72`). Still needs Railway env vars + `web/` staging deploy. Next session: read this file first, update it last.*
