...
- **Tailor WYSIWYG preview + match sidebar (this session)** — Tailor results now use `TailorPreviewPane` → shared `AnnotatedResumePanel` + `useHtmlPdfExport` (same WYSIWYG stack as Analyze) instead of a separate hidden `ResumeDocumentView` export path. `DetailedRatingsView` split into `TailorMatchSidebar` + `TailorMatchDetail` for the two-column match workspace. `useUploadResume` uses `mergeAnalyzeApiJson` and surfaces `extractedText` / `resumeHeader` when the backend provides them. Upload still returns legacy `text` until `/api/upload-resume` gets vision-synthesized parity with analyze-upload.
